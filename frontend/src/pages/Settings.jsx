...
export default function Settings({ open, onClose, onSubmit }) {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex justify-center items-center transition-colors duration-200 ${open ? "visible bg-black/50" : "invisible"}`}>
            <div onClick={(e) => e.stopPropagation()} className={`relative bg-white rounded-xl shadow p-6 w-2xl h-161 flex flex-col gap-2.5 transition-all duration-200 dark:bg-slate-950 dark:border dark:border-gray-600 ${open ? "scale-100 opacity-100" : "scale-125 opacity-0"}`}>
                ...
                <div className="border rounded-md border-gray-500/30 p-4 h-auto overflow-auto">
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 px-3">
                        <div className="flex flex-col justify-center gap-1">
                            <p className="text-sm font-medium dark:text-white">Current Password</p>
                            <input type={showPassword ? "text" : "password"} value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Enter current password" className="border-2 border-gray-400/30 rounded text-sm p-2.5 w-full dark:text-gray-300" />
                        </div>
                        <div className="flex flex-col justify-center gap-1">
                            <p className="text-sm font-medium dark:text-white">New Password</p>
                            <input type={showPassword ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password" className="border-2 border-gray-400/30 rounded text-sm p-2.5 w-full dark:text-gray-300" />
                        </div>
                        <div className="flex flex-col justify-center gap-1">
                            <p className="text-sm font-medium dark:text-white">Confirm New Password</p>
                            <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm new password" className="border-2 border-gray-400/30 rounded text-sm p-2.5 w-full dark:text-gray-300" />
                        </div>
                        <button
