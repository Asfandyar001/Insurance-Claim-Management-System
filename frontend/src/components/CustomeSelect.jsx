...
export default function CustomSelect({ options, onChange, icon, width = "w-48", height = "2.5" }) {
    ...
    return (
        <div ref={ref} className={`relative ${width}`}>
            {/* Selected */}
            <div
                className={`flex items-center px-3 py-${height} border border-gray-300 dark:border-gray-800 text-sm rounded-md cursor-pointer`}
                    ...
                    {options.map((option, idx) => (
                        <li
                            ...
                            className={`px-3 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-white
                                ${option === selected ? "bg-gray-200 dark:bg-gray-800" : "bg-white dark:bg-slate-950"}
                                ${idx === 0 ? "rounded-t-lg" : ""}
